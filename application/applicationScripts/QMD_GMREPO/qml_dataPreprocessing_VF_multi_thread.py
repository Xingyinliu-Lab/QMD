...
for i in genuslist.index:
    ...
    try:
        ...
        if controlobs>1:
            res_detection_ratio.loc[count, 'controlAbundance'] = np.nanmedian(controlgdata)
        else:
...
def cal_gdiff(glist):
    ...
    for i in glist.index:
        ...
        try:
            ...
            glist.loc[i, 'treat_logged_mean'] = mean_t
            glist.loc[i, 'control_logged_mean'] = mean_c
            glist.loc[i, 'logged_mean_diff'] = mean_diff
            glist.loc[i, 'logged_mean_diff05'] = lower_bound
            glist.loc[i, 'logged_mean_diff95'] = upper_bound
            if len(gdiff)>0:
...
if __name__ == '__main__':
    cl = np.array_split(np.asarray(range(len(genuslist))), processers, axis=0)
    res_list = []

    gdiff_tmp, glist_tmp = cal_gdiff(genuslist.loc[cl[0],])

    p = Pool(processers)
    for i in range(processers):
        res_list.append(p.apply_async(cal_gdiff, args=(genuslist.loc[cl[i],],)))
    p.close()
    p.join()
    genuslist = pd.concat([i.get()[1] for i in res_list])
    gdiff = np.hstack([i.get()[0] for i in res_list])
