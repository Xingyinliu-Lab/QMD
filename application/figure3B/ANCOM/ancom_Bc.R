...
data<- read.table('PRJNA763023_single_abs.csv', header = TRUE, sep = ",")
data[is.na(data)]<-0
id<-1:(dim(data)[1])
id<-paste('sample-',id,sep='')
data$sample_id<-id
rownames(data)<-id
meta_data<-data[,c('sample_id','group')]
sample.var = "sample_id";
group.var = "group"; 
zero.cut = 0.90;
lib.cut = 1000;
neg.lb = TRUE
otu_data<-data[,!names(data) %in% c('sample_id','group')]
...
resfilename=paste('ANCOM_BC_res1.csv',sep='')
res2filename=paste('ANCOM_BC_res2.csv',sep='')
