## the code is to prepare simulation instances from Global patterns series
...
fileplace='originalDataPool/GP/'
minimun_taxa_relative_abundance=0.001
minimun_taxa_relative_abundance_obs=5
for ooo in range(500):
    i=ooo+1
    oriData=pd.read_csv(fileplace+'gp_'+str(i)+'_relative_otu.csv',header=0,index_col=0)
    ...
    oriData.columns='taxa'+oriData.columns
    ...
    for taxa in taxalist:
        ...
        count=count+1
    # store generated absolute abundance
    absolute_abundance_Dataset = pd.concat([controlDataset, treatDataset], axis=0)
    absolute_abundance_Dataset.to_csv(fileplace + 'gp_' + str(i) + '_absolute_abundance_simed.csv', index=False)
    # store generated relative abundance
    ...
    simDataset.to_csv(fileplace+'gp_'+str(i)+'_relative_otu_simed.csv',index=False)
    # store change record
    changRecord.to_csv(fileplace+'gp_'+str(i)+'_changed_record_simed.csv',index=False)
