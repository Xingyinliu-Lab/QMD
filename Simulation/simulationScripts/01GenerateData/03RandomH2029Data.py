## the code is to prepare simulation instances from H2029 series
...
fileplace='originalDataPool/H2029/'
minimun_taxa_relative_abundance=0.001
minimun_taxa_relative_abundance_obs=10

for ooo in range(500):
    i=ooo+1
    oriData = pd.read_csv(fileplace + 'H2029' + '_relative_otu.csv', header=0, index_col=0)
    ...
    absolute_abundance_Dataset.to_csv(fileplace + 'H2029_' + str(i) + '_absolute_abundance_simed.csv', index=False)
    # relative abundance
    ...
    simDataset.to_csv(fileplace+'H2029_'+str(i)+'_relative_otu_simed.csv',index=False)
    # store change record
    changRecord.to_csv(fileplace+'H2029_'+str(i)+'_changed_record_simed.csv',index=False)
