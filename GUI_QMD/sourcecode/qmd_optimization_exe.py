...
## the code mwutest is extracted from scipy.stats import mannwhitneyu
## the code fdrcorrection is extracted from statsmodels.stats.multitest
## the reason of extraction code instead of import scipy or statsmodel is that in this way we can reduce the file size of QMD.exe packaged by pyinstaller
## we have tested the reliablity and accuracy of the exacted code. It performs as well as its original copy.
import mwutest
import fdrcorrection

def qmd_optimization(fileplace, data_preprocessing_place, control_label, treat_label, group_label, permu_loops, fdr, os_p, plot, confidence_level):
    fileplace = fileplace
    ...
    for taxa in genusdata.columns:
        ...
        count = count + 1
        _, qmd_pvalue = mwutest.mannwhitneyu_np(
    ...
    _, qvaluelist = fdrcorrection.fdrcorrection_np(
        tmpPvalue['value'], alpha=0.05, method='indep', is_sorted=False)
